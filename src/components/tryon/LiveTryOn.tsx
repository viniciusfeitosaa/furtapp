...
import { resolveHairTryOnProvider } from "@/lib/tryon/config";
...
import {
  CLINICAL_STYLES,
  type ClinicalStyleId,
} from "@/lib/tryon/styleCatalog";
import { whatsappUrl } from "@/lib/site";

type Provider = ReturnType<typeof resolveHairTryOnProvider>;

export function LiveTryOn() {
  const provider: Provider = resolveHairTryOnProvider();
  const isBanuba = provider === "banuba";

  const { videoRef, state, error, start, stop } = useCamera();
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const banubaHostRef = useRef<HTMLDivElement | null>(null);
  const engineRef = useRef<HairTryOnEngine | null>(null);
  const rafRef = useRef(0);
  const styleRef = useRef(isBanuba ? "classico" : "natural");
  const intensityRef = useRef(0.7);
  const [intensity, setIntensity] = useState(70);
  const [styleId, setStyleId] = useState(isBanuba ? "classico" : "natural");
  ...
  useEffect(() => {
    let cancelled = false;
    const engine = createHairTryOnEngine(provider);
      ...
      try {
        ...
        if (!cancelled) {
          setModelError(
            e instanceof Error
              ? e.message
              : "Não foi possível carregar o motor de try-on. Recarregue a página.",
          );
        }
      }
    })();
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (!engine || engine.kind !== "segment-tint" || video.readyState < 2) {
  ...
  }, [state, videoRef, isBanuba]);

  // Propagar estilo/intensidade no Banuba
  useEffect(() => {
    const engine = engineRef.current;
    if (!isBanuba || !sdkLive || !engine || engine.kind !== "sdk-style") return;
    void engine.setStyle(styleId, intensity / 100);
  }, [styleId, intensity, isBanuba, sdkLive]);

  const startBanuba = async () => {
    const engine = engineRef.current;
    const host = banubaHostRef.current;
    if (!engine || engine.kind !== "sdk-style" || !host) return;
    try {
      await engine.mount(host);
      await engine.startCamera();
      await engine.setStyle(styleRef.current, intensityRef.current);
      setSdkLive(true);
      setHasHair(true);
    } catch {
      setModelError(
        "Não foi possível iniciar a câmera Banuba. Verifique permissões e o token.",
      );
    }
  };
  ...
  const live = isBanuba ? sdkLive : state === "live";

  const activeTint =
    HAIR_TINT_PRESETS.find((p) => p.id === styleId) ?? HAIR_TINT_PRESETS[0]!;
  const activeStyle =
    CLINICAL_STYLES.find((p) => p.id === styleId) ?? CLINICAL_STYLES[1]!;
        ...
        <div className="relative aspect-[3/4] w-full bg-black sm:aspect-video">
          {!isBanuba ? (
            ...
            <div
              ref={banubaHostRef}
              id="banuba-webar"
              className="absolute inset-0 flex items-center justify-center [&_canvas]:max-h-full [&_canvas]:max-w-full [&_canvas]:object-cover"
              aria-label="Try-on Banuba ao vivo"
            />
          )}
          ...
          {!live ? (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-5 bg-[#060810]/px-6 text-center">
              ...
              <p className="font-serif-body max-w-md text-sm text-white/70 sm:text-base">
                {isBanuba
                  ? "Motor Banuba WebAR — estilos/cor com segmentação comercial. Nada é gravado pelo site."
                  : "A câmera detecta o cabelo real (segmentação por IA) e aplica reforço de tom/densidade — sem chapéu 3D. Nada é gravado ou enviado."}
              </p>
              <button
                type="button"
                onClick={() => {
                  if (isBanuba) void startBanuba();
                  else void start();
                }}
      ...
      <div className="mt-8 space-y-8">
        <div>
          <p className="mb-3 text-[0.7rem] font-semibold tracking-wide text-white/70 uppercase">
            {isBanuba
              ? "Estilo clínico (Banuba)"
              : "Tom / reforço ilustrativo"}
          </p>
          {isBanuba ? (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
              {CLINICAL_STYLES.map((s) => {
                const on = s.id === styleId;
                return (
                  <button
                    key={s.id}
                    type="button"
                    onClick={() => setStyleId(s.id as ClinicalStyleId)}
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
              {HAIR_TINT_PRESETS.map((s) => {
                const on = s.id === styleId;
                return (
                  <button
                    key={s.id}
                    type="button"
                    onClick={() => setStyleId(s.id as HairTintId)}
                    className={`border px-3 py-3 text-left transition-colors ${
                      on
                        ? "border-brand-gold bg-brand-gold/15 text-white"
                        : "border-white/15 text-white/70 hover:border-white/35 hover:text-white"
          ...
          <p className="mt-3 text-sm text-white/50">
            ...
            <span className="text-brand-gold">
              {isBanuba ? activeStyle.label : activeTint.label}
            </span>{" "}
            — {isBanuba ? activeStyle.blurb : activeTint.blurb}
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
          ...
          <div className="flex flex-col gap-4">
            <p className="text-sm leading-relaxed text-white/55">
              {isBanuba
                ? "Fase 2: Banuba. Com assets de estilo (.zip), troca o corte; sem STYLES_BASE, usa Hair.color no effect Makeup. Não é prognóstico cirúrgico."
                : "Fase 1: segmentação do cabelo existente. Estilos reais via Banuba quando o token trial estiver configurado. Não é prognóstico cirúrgico."}
            </p>
            {!hasHair && live && !isBanuba ? (
              <p className="text-sm text-brand-gold/90">
                Se a área estiver raspada ou a luz for fraca, a máscara pode
                falhar — estilos completos virão com o SDK.
              </p>
            ) : null}
            <div className="flex flex-wrap gap-3">
              ...
              <a
                href={whatsappUrl(
                  isBanuba
                    ? `Olá! Testei o estilo "${activeStyle.label}" no try-on ao vivo e gostaria de agendar minha avaliação.`
