...
export function LiveTryOn() {
  ...
  const styleRef = useRef<HairTintId>("natural");
  const intensityRef = useRef(0.7);
  const [intensity, setIntensity] = useState(70);
  const [styleId, setStyleId] = useState<HairTintId>("natural");
  ...
  useEffect(() => {
    let cancelled = false;
    const engine = createMediaPipeHairSegmentEngine();
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setModelError(
            "Não foi possível carregar o modelo de segmentação de cabelo. Recarregue a página.",
          );
        }
      }
    })();
          ...
          {state !== "live" ? (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-5 bg-[#060810]/px-6 text-center">
              ...
              <p className="font-serif-body max-w-md text-sm text-white/70 sm:text-base">
                A câmera detecta o cabelo real (segmentação por IA) e aplica um
                reforço de tom/densidade — sem chapéu 3D flutuando. Nada é
                gravado ou enviado.
              </p>
              <button
                type="button"
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
          ...
          <div className="flex flex-col gap-4">
            <p className="text-sm leading-relaxed text-white/55">
              Fase 1 do plano técnico: segmentação do cabelo existente. Cortes /
              “perucas” reais entram na Fase 2 (SDK Banuba ou DeepAR). Não é
